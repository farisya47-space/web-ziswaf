  ...
  <div class="flex-1 p-5 md:p-8">
    ...
    <div v-if="currentView === 'list'">
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          <h3 class="font-semibold text-lg">Daftar Pemasukan</h3>
            ...
            <tbody class="divide-y divide-border">
              ...
              <tr v-else-if="!transactions.length">
                <td colspan="6" class="p-8 text-center text-secondary text-sm">
                  {{ searchQuery ? `Tidak ada hasil untuk "${searchQuery}"` : 'Tidak ada data pemasukan' }}
                </td>
              </tr>
              <tr v-for="trx in transactions" :key="trx.id" class="hover:bg-muted/30 transition-all" :class="loading ? 'opacity-50' : ''">
                ...
                <td class="p-4 font-semibold text-success">{{ formatCurrency(trx.amount) }}</td>
        ...
        <div class="sm:hidden divide-y divide-border">
          ...
          <div v-for="trx in transactions" :key="trx.id" class="p-4 flex items-start justify-between gap-3" :class="loading ? 'opacity-50' : ''">
            <div class="flex flex-col gap-1 min-w-0">
              ...
              <span class="font-semibold text-success text-sm">{{ formatCurrency(trx.amount) }}</span>
    ...
    <div v-else class="max-w-2xl mx-auto">
      <div class="bg-white border border-border rounded-2xl shadow-sm overflow-hidden">
        <div class="p-6 md:p-8 border-b border-border">
          <h3 class="font-bold text-lg text-foreground">{{ currentView === 'add' ? 'Tambah' : 'Edit' }} Pemasukan</h3>
          <p class="text-sm text-secondary mt-1">{{ currentView === 'add' ? 'Lengkapi form untuk menambahkan pemasukan baru' : 'Ubah data transaksi pemasukan' }}</p>
        </div>
...
async function loadTransactions() {
  loadError.value = ''
  try {
    await store.fetchTransactions('incomes', { page: currentPage.value, per_page: perPage, search: searchQuery.value })
  } catch (error) {
...
async function showAddForm() {
  ...
  errors.value = {}
  serverError.value = ''
  await categoryStore.fetchCategories({ type: 'income', per_page: 100 })
...
async function saveTransaction() {
  ...
  try {
    ...
    if (isAdd) { await store.createTransaction('incomes', payload) }
    else { await store.updateTransaction('incomes', form.value.id, payload) }
    currentView.value = 'list'
    showToast(`Pemasukan berhasil ${isAdd ? 'ditambahkan' : 'diperbarui'}`)
...
async function confirmDelete(id) {
  try {
    await store.deleteTransaction('incomes', id)
